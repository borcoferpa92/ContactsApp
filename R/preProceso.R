...
#' @return
#' @import logging
preProceso <- function(datos, config){
...
splitearDatos <- function(datos, config){
  ...
  # Evaluados por estado BUENOS/MALOS.
  ...
  #Evaluar abiertos negativos.
  abiertos <- datos[datos[, config$columnas$target] == 'Open', ]
  
  openNegativos <- c('Contacto no válido', 'Descartada - Demasiados intentos')
