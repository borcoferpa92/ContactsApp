...
seleccionColumnas <- function(datosSplit, config) {
  ...
  #Anhadimos las columnas que son necesarias guardar
  colNecesarias <- c(variablesToModel, config$columnas$ID, config$columnas$llamada)
  
  #El dataframe entrenado tiene la columna target, pero el dataframe de predict no por lo que la anhadimos
